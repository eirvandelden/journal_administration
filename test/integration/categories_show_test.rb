...
class CategoriesShowTest < ActionDispatch::IntegrationTest
  ...
  test "show renders split amount and split category for split transactions" do
    ...
    assert_response :success
    assert_match split_row_pattern_for(amount: 10, category: categories(:bakery),
                                       note: transactions(:debit_grocery).note),
                 response.body
  end
  ...
  test "show includes the remainder amount for the original category" do
    ...
    assert_response :success
    assert_match split_row_pattern_for(amount: 40, category: categories(:supermarket),
                                       note: transactions(:debit_grocery).note),
                 response.body
  end
