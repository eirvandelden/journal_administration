...
class DateRangeTest < ActiveSupport::TestCase
  ...
  test "from_filter with month_to_date returns beginning of month to now" do
    range = DateRange.from_filter("month_to_date")

    assert_equal Time.current.beginning_of_month.to_date, range.start_date.to_date
    assert_in_delta Time.current, range.end_date, 5.seconds
  end
  ...
  test "FILTERS contains all valid filter options" do
    ...
    assert_includes DateRange::FILTERS, "month_to_date"
  end
end
